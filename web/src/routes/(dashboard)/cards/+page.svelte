<script lang="ts">
	...
	$effect(() => {
		...
		[searchTerm, selectedPriority, selectedTopicId, selectedDeckId, selectedTag];
		currentPage = 1;
	});
	...
	const filteredDecks = $derived(
		selectedTopicId
			? data.decks.filter((deck) => deck.topicId === parseInt(selectedTopicId))
			: data.decks
	);
	...
	const filteredCards = $derived(() => {
		...
		if (selectedTopicId) {
			filtered = filtered.filter((card) => card.topicId === parseInt(selectedTopicId));
		}

		if (selectedDeckId) {
			filtered = filtered.filter((card) => card.deckId === parseInt(selectedDeckId));
		}
...
<svelte:head>
	<title>Cards - Cadence Cards</title>
</svelte:head>
...
<div>
	...
	<div class="mb-6 overflow-hidden rounded-lg bg-white shadow">
		<div class="px-4 py-5 sm:p-6">
			...
			<div class="flex flex-col gap-4 sm:flex-row">
				...
				<!-- Topic Filter - fixed width -->
				<div class="w-full sm:w-40">
					<label for="topic" class="mb-1 block text-sm font-medium text-gray-700">Topic</label>
					<select
						id="topic"
						bind:value={selectedTopicId}
						onchange={() => (selectedDeckId = '')}
						class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 font-sans shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:outline-none sm:text-sm"
					>
				...
				<!-- Deck Filter - fixed width -->
				<div class="w-full sm:w-40">
					<label for="deck" class="mb-1 block text-sm font-medium text-gray-700">Deck</label>
					<select
						id="deck"
				...
				<div class="w-full sm:w-40">
					...
					>
						...
						{#each data.availableTags as tag (tag)}
						...
						{#each paginatedCards() as card (card.id)}
							<tr class="hover:bg-gray-50">
								...
								<td class="px-4 py-3 text-sm text-gray-500">
									<a href={`/topics/${card.topicId}`} class="text-indigo-600 hover:text-indigo-900">
										{card.topicName}
									</a>
									<span class="text-gray-400">/</span>
									<a href={`/decks/${card.deckId}`} class="text-indigo-600 hover:text-indigo-900">
										{card.deckName}
									</a>
								</td>
								<td class="px-4 py-3 text-sm">
	...
	<ConfirmDialog
		...
		onClose={() => (deleteConfirm = { isOpen: false, id: null })}
